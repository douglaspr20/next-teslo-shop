...
const getPaypalBearrToken = async (): Promise<string | null> => {
...
const payOrder = async (req: NextApiRequest, res: NextApiResponse<Data>) => {
  try {
    const paypalBearerToken = await getPaypalBearrToken();
    ...
    if (dbOrder.total !== Number(data.purchase_units[0].amount.value)) {
      await db.disconnect();
      return res.status(400).json({ message: "the amounts are not equal" });
    }
