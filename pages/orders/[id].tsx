...
const OrderPage: NextPage<Props> = ({ order }) => {
  ...
  const [orderAlertPay, setOrderAlertPay] = useState<OrderAlert>({
    type: "error",
    message: "",
  });
  ...
  const onOrderCompleted = async (details: OrderResponseBody) => {
    if (details.status !== "COMPLETED") {
      return alert("No se ha realizado el pago");
    }
    setIsPaying(true);
  ...
  return (
    <ShopLayout title="Order Resume" pageDescription={"Order Resume"}>
      {orderAlertPay.message !== "" && (
        <Alert severity={orderAlertPay.type}>{orderAlertPay.message}</Alert>
      )}
